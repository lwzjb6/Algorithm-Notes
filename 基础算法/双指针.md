...
因为是在单调递增序列中找到大于等于`x`的最小值，因此可以用STL中的lower_bound实现
...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        for(int i = 0; i < nums.size(); i++) {
            if (i > 0 && nums[i] == nums[i - 1]) continue; // 说明之前相同的元素已经考虑过了
            int j = i + 1, k = nums.size() - 1;
            while(j < k) {
                if(j > i + 1 && nums[j] == nums[j - 1]) {j++; continue;} //这边从i+2开始判断是因为避免第一个元素因为和i指针指向的元素一样就直接跳过。
